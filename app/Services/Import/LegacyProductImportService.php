...
class LegacyProductImportService
{
    ...
    public function importCarrierRow(array $row): void
    {
        ...
        try {
            DB::transaction(function () use ($sku, $row) {
                ...
                $typeCode = $this->resolveTypeCode($row['Typ'] ?? '');
        ...
        foreach (self::ATTR_MAP as $code => $spec) {
            ...
            if ($spec['type'] === 'number') {
                ...
                ProductAttributeValue::updateOrCreate(
                    ['product_id' => $product->id, 'attribute_id' => $this->attrId($code)],
    ...
    /**
     * Normalize old Typ column to a product_type code.
     * Extend this map to cover all values that appear in production data.
     */
    private function resolveTypeCode(string $typ): ?string
    {
        return match (mb_strtolower(trim($typ))) {
            'battery', 'baterie', 'bat'       => 'battery',
            'charger', 'nabijecka', 'chg',
            'adaptér', 'adapter', 'adp'       => 'charger',
